...
export default function Login() {
  ...
  const handleAssinar = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('pagamentos').insert({
        ...
        observacoes: `Pagamento solicitado via modal PIX - Negócio: ${negocio}`
      })
  ...
  if (tela === 'plano') return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-dark-900">
      ...
      {/* MODAL PIX */}
      {showModalPix && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowModalPix(false)}>
          <div className="bg-dark-800 rounded-2xl border border-dark-500/50 p-8 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-bold text-white mb-6 text-center">Pagamento via PIX</h3>

            <div className="bg-primary-500/10 border border-primary-500/30 rounded-2xl p-5 text-center mb-5">
              <p className="text-sm text-primary-400 font-semibold mb-1">Valor do Plano Mensal</p>
              <p className="text-4xl font-extrabold text-primary-400">R$ 29,00</p>
            </div>

            <div className="bg-dark-700 rounded-2xl p-5 mb-5 space-y-4 border border-dark-500/50">
              <h4 className="font-bold text-white text-sm">📋 Dados para pagamento</h4>
              <div>
                <p className="text-xs text-gray-500 mb-1">Chave PIX (Aleatória)</p>
                <div className="flex items-center gap-2 bg-dark-600 border border-dark-500 rounded-xl p-3">
                  <span className="text-xs font-mono text-gray-300 flex-1 break-all">{pixKey}</span>
                  <button onClick={copiarPix} className="bg-primary-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg whitespace-nowrap">Copiar</button>
                </div>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Nome</p>
                <p className="text-sm font-semibold text-gray-200">{pixName}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Banco</p>
                <p className="text-sm font-semibold text-gray-200">{pixBank}</p>
              </div>
            </div>

            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-5 text-xs text-amber-300">
            ...
            <a href="https://wa.me/5516992383821?text=Olá!%20Realizei%20o%20pagamento%20do%20Parcelyx%20no%20valor%20de%20R%24%2029%2C00.%20Segue%20o%20comprovante."
              ...
              className="w-full py-3.5 bg-pix-500 hover:bg-pix-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 mb-3">
            ...
            <button onClick={() => setShowModalPix(false)}
